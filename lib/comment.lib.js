...
const commentOnAnswer = async (commentDetails) => {
  ...
  try {
    const comment = await AnswerComments.findOrCreate({ user_id, answer_id, comment_text });
    return comment;
  } catch (error) {
    throw new Error(`${error.message}`);
  }
...
const editCommentOnAnswer = async (commentDetails) => {
  const {
    user_id, comment_id, comment_text,
  } = commentDetails;
  try {
    const comment = await fetchAnswerComment({ user_id, comment_id });
    if (comment) {
      comment.comment_text = comment_text;
      await comment.save();
      return comment;
